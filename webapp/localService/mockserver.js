sap.ui.define([
				...
				if (navigationProperty && entityType) {
					...
					navPropData.forEach(function (iteratedDataPoint) {
						...
						for (i = 0; i < referentialConstraints.length; i++) {
							...
							if ((multiplicity && iteratedDataPoint[dependentProperty] !== dataPoint[principalProperty]) || !multiplicity && dataPoint[
									dependentProperty] !== iteratedDataPoint[principalProperty]) {
								res = false;
							}
						}
			...
			innerFormulaCalculation.getCalculatedProperties = function (entity, property, data, calculatedProperties, modelHelper) {
				...
				properties.forEach(function (calculatedProperty) {
					...
					if (calculatedProperties.filter(function (theProperty) {
							...
							return thePropName && thePropName.toLowerCase() === addedPropName && addedPropName.toLowerCase() && thePropParent &&
								thePropParent.toLowerCase() === addedPropParent && addedPropParent.toLowerCase();
						}).length === 0) {
						if (propCalc && propCalc.calculation) {
							...
							propCalc.inputProperties.forEach(function (inputProperty) {
								...
								if (subPropCalc && subPropCalc.calculation) {
									calculatedProperties = innerFormulaCalculation.getCalculatedProperties(entity, property, data, calculatedProperties,
										modelHelper);
								}
							});
						}
			...
			innerFormulaCalculation.generateResult = function (formulaObj, data, dataPoint, entity, modelHelper) {
				...
				case "property":
					...
					if (formulaObj[0].navPropId) {
						...
						result = null || (navigationProperty && innerFormulaCalculation.getNavPropValues(data, dataPoint, formulaObj[0],
							navigationProperty, modelHelper));
			...
			innerFormulaCalculation.calculateData = function (entity, data, modelHelper) {
				...
				for (var i = 0; i < properties.length; i++) {
					...
					if (calcProp && calcProp.calculation) {
						calculatedProperties = innerFormulaCalculation.getCalculatedProperties(entity, properties[i], data, calculatedProperties,
							modelHelper);
					}
				}
	...
	var oMockServer,
		_sAppModulePath = "com.sap.build.standard.covidSafePass/";
	return {
